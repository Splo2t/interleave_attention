...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "(mmlu/kmmlu/csatqa/click) 콤마로 분리된 과목 목록. "
            "csatqa 예: WR,GR,RCS,RCSS,RCH,LI. "
            "click 예: language,culture,KL_Grammar,KC_History"
        ),
    )
    ...
    parser.add_argument("--kobest_tasks", type=str, default="", help="(kobest) tasks: boolq,copa,hellaswag,sentineg,wic")
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "(hellaswag/openbookqa) 평가 split override. "
            "기본값: hellaswag=validation, openbookqa=test"
        ),
    )
    parser.add_argument("--rerank_data", type=str, default="", help="(korean_rerank) reranking candidate JSONL path")
    parser.add_argument("--rerank_max_candidates", type=int, default=100, help="(korean_rerank) query당 최대 후보 수")
    parser.add_argument("--rerank_yes_label", type=str, default=" 예", help="(korean_rerank) positive continuation label")
    parser.add_argument("--rerank_no_label", type=str, default=" 아니오", help="(korean_rerank) negative continuation label")
    parser.add_argument(
        "--rerank_score_mode",
        type=str,
        default="diff",
        choices=["diff", "norm_diff", "yes", "yes_norm"],
        help="(korean_rerank) candidate score: yes-no loglik diff, normalized diff, yes LL, or normalized yes LL",
    )
    parser.add_argument(
        "--rerank_prompt_template",
        type=str,
        default="",
        help="(korean_rerank) Python format template with {query} and {passage}",
    )
    parser.add_argument(
        "--rerank_num_fewshot",
        type=int,
        default=0,
        help="(korean_rerank) 실제 rerank candidate 데이터에서 뽑아 prompt 앞에 붙일 few-shot 예시 수",
    )
    parser.add_argument(
        "--rerank_fewshot_data",
        type=str,
        default="",
        help="(korean_rerank) few-shot 예시를 뽑을 JSONL. 비우면 rerank_data에서 현재 query를 제외하고 뽑음",
    )
    parser.add_argument(
        "--rerank_fewshot_seed",
        type=int,
        default=42,
        help="(korean_rerank) few-shot 예시 샘플링 seed",
    )
    parser.add_argument(
        "--model_arch",
        type=str,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.out_json:
        ...
        with open(args.out_json, "w", encoding="utf-8") as f:
            json.dump(benchmark_run.results, f, ensure_ascii=False, indent=2)
